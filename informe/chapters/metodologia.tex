...
\subsection{Zhao et al. (2022) y Portell \& Lourenço (2024): Tratamiento del Time-Dependency}
...
\begin{enumerate}
    \item \textbf{Tamaño reducido}: se selecciona una submuestra estratificada de 40 pasajeros del turno AM centrada en el pico operativo de las 04:50, preservando la distribución de categorías (aprox.\ $14/35/45/6\%$) y la dispersión geográfica observada en la instancia original.
    \item \textbf{Flota homogénea}: se considera únicamente vehículos tipo \textsc{Common} ($Q=3$, $F=2\,200$ CLP), eliminando temporalmente la dimensión de selección de tipo. Esto alinea el Caso Base con la configuración de \cite{Pilati2025}, facilitando la comparación directa de operadores ALNS sin confundirlos con efectos de heterogeneidad.
    \item \textbf{Filtrado de \textit{outliers}}: se descartan observaciones de la muestra de tiempos reales con $\texttt{real\_time}$ fuera del rango intercuartil extendido (percentiles 5--95) antes de entrenar el estimador temporal, reduciendo el sesgo de casos anómalos (tiempos de 6 s o 5\,881 s identificados en el Capítulo~\ref{ch:problema}).
    \item \textbf{Dependencia temporal preservada}: se mantiene la estructura $\tau_{ij}(t)$ con las tres matrices horarias (04, 05, 06), puesto que el enunciado exige rutas ``lo más acertadas posibles en sus tiempos de viaje''. Eliminar el componente TD invalidaría la tesis central del proyecto.
\end{enumerate}

\subsection{Línea Base y KPIs Reportados}
Como \textit{benchmark} comparativo se implementa una \textbf{heurística miope}: asigna cada pasajero, en orden ascendente por $\sigma_i$, al vehículo Common activo que minimice el costo marginal dentro de sus ventanas factibles; si ninguno acepta, abre un nuevo vehículo. Además del costo objetivo \eqref{eq:obj}, se reportarán (siguiendo a \cite{Pilati2025}) tiempo de espera promedio por pasajero, balance de jornada de conductores (mín/mediana/máx por ruta), ocupación promedio y \textit{gap} relativo respecto a la línea base miope.
...
\multicolumn{2}{l}{\textit{Destrucción}} \\ \addlinespace
...
\multicolumn{2}{l}{\textit{Reparación}} \\ \addlinespace
...
Regret-2 / Regret-3 Insertion & Prioriza solicitudes cuyo costo se dispara si no se insertan en su mejor posición; Regret-3 resulta útil para pasajeros críticos (categoría 4, ventana de 5 min). \\ \addlinespace
Zero-load Insertion & Novel de \cite{Pilati2025}: inserta en posiciones donde el vehículo está vacío, facilitando simultáneamente factibilidad de capacidad y de categoría. \\ \bottomrule
\end{tabular*}
\end{table}

La selección de operadores se realiza por \textit{ruleta} con probabilidades proporcionales a los pesos $w_i$, actualizados cada $t_{\mathrm{seg}}$ iteraciones mediante $w_{i,j+1}=w_{i,j}(1-\rho)+\rho\,\pi_i/\theta_i$, donde $\pi_i$ acumula los scores $\sigma_1=10$ (nuevo \textit{global best}), $\sigma_2=2$ (nueva referencia) o $\sigma_3=0$ y $\theta_i$ es el número de usos del operador en el segmento. Se siguen los valores de \cite{Pilati2025}: $\rho=0{,}1$ y $\delta\sim U(0{,}05,0{,}10)$; los tiempos $t_{\mathrm{tot}}$ ($\approx$30 min CPU), $t_{\mathrm{seg}}$ ($\approx$500 iter) y $t_{\mathrm{last}}$ ($\approx$2\,000 iter sin mejora) se calibrarán empíricamente sobre el Caso Base en la Entrega 3.

\subsection{Criterio de Aceptación y Parada}
La solución candidata $s'$ se acepta como nuevo incumbente $s_{\text{best}}$ sólo si es factible y $f(s')<f(s_{\text{best}})$. Como referencia de trabajo $s^*$, se acepta si $f(s')<f(s^*)$; si no hay mejora durante $t_{\mathrm{last}}$ iteraciones, $s'$ pasa a ser la nueva referencia (diversificación forzada). Si $c(s')\leq 1{,}02\cdot c(s^*)$, se aplica búsqueda local intra-ruta (swap/relocate) para intensificación, siguiendo el híbrido de \cite{Zhao2022}. El algoritmo termina al alcanzar $t_{\mathrm{tot}}$.

\section{Fortalezas y Debilidades}

Como \textbf{fortalezas}, el MILP \eqref{eq:obj}--\eqref{eq:r14} es coherente con el enunciado y los datos, habilita el uso de Gurobi como oráculo sobre el Caso Base, y la función de evaluación \eqref{eq:eval-ours} absorbe infactibilidades transitorias sin comprometer la factibilidad final, esencial en un espacio fragmentado por restricciones sindicales; además, el portafolio de operadores incluye uno propio (\textit{Category-violation Removal}) inexistente en la literatura revisada, y la adopción de parámetros validados en \cite{Pilati2025} reduce el espacio de calibración. Como \textbf{debilidades}, la representación escalonada de $\tau_{ij}(t)$ puede subestimar la volatilidad real del tráfico y el error del XGBoost se propaga al modelo; las restricciones se asumen duras, por lo que una infactibilidad en el Caso Base (por ejemplo, categoría 4 aislada) obligaría a relajaciones documentadas; la heurística constructiva aleatorizada admite soluciones iniciales infactibles que el ALNS debe reparar antes de mejorar el costo; y el peso $\phi$ del término sindical no tiene calibración previa en la literatura.
